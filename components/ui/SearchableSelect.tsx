...
export default function SearchableSelect({
    ...
    return (
        <div className="flex flex-col gap-1 w-full relative" ref={wrapperRef}>
            ...
            {/* Dropdown Menu */}
            {isOpen && (
                <div className="absolute top-full left-0 w-full mt-2 bg-[#1a1a1a] border border-white/10 rounded-lg shadow-xl z-50 overflow-hidden flex flex-col max-h-60 animate-in fade-in zoom-in-95 duration-100">
                    {/* Search Input */}
                    <div className="p-2 border-b border-white/10 sticky top-0 bg-[#1a1a1a]">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                className="w-full bg-white/5 border border-white/10 rounded-md py-1.5 pl-9 pr-3 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:border-purple-500/50"
                                placeholder="Ara..."
                                value={search}
                            ...
                            filteredOptions.map((option) => (
                                <div
                                    ...
                                    className={`
                                        ...
                                        ${value === option ? 'bg-purple-500/20 text-purple-300' : 'text-gray-300 hover:bg-white/5 hover:text-white'}
                                    `}
                                    onClick={() => {
                                        onChange(option);
                                        setIsOpen(false);
